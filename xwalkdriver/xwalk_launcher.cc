...
Status LaunchRemoteXwalkSession(
  ...
  xwalk->reset(new XwalkRemoteImpl(devtools_http_client.Pass(),
...
Status LaunchDesktopXwalk(
  ...
  scoped_ptr<XwalkDesktopImpl> xwalk_desktop(
      new XwalkDesktopImpl(devtools_http_client.Pass(),
                            devtools_websocket_client.Pass(),
                            *devtools_event_listeners,
...
Status LaunchAndroidXwalk(
  ...
  xwalk->reset(new XwalkAndroidImpl(devtools_http_client.Pass(),
                                      devtools_websocket_client.Pass(),
                                      *devtools_event_listeners,
                                      port_reservation.Pass(),
                                      device.Pass()));
...
Status LaunchXwalk(
  ...
  if (capabilities.IsAndroid()) {
    ...
    if (port_status.IsError())
      return Status(kUnknownError, "cannot reserve port for Xwalk",
                    port_status);
    if (capabilities.specialDebugPort()) {
      port = capabilities.debug_port;
    }
...
Status ProcessExtension(const std::string& extension,
  ...
  if (is_crx_file) {
    // Assume a CRX v2 file - see https://developer.xwalk.com/extensions/crx.
...
Status PrepareUserDataDir(
  ...
  base::FilePath default_dir =
      user_data_dir.AppendASCII(xwalk::kInitialProfile);
  ...
  base::FilePath preferences_path =
      default_dir.Append(xwalk::kPreferencesFilename);
  ...
  base::FilePath local_state_path =
      user_data_dir.Append(xwalk::kLocalStateFilename);
  ...
  if (base::WriteFile(
          user_data_dir.Append(xwalk::kFirstRunSentinel), "", 0) != 0) {
