...
#include "xwalk/test/xwalkdriver/xwalk_constants.h"
...
TEST(ProcessExtensions, GenerateIds) {
  ...
  ASSERT_EQ("xwalk-extension://llphabdmknikmpmkioimgdfbohinlekl/"
            "_generated_background_page.html", bg_pages[0]);
  ASSERT_EQ("xwalk-extension://dfdeoklpcichfcnoaomfpagfiibhomnh/"
            "_generated_background_page.html", bg_pages[1]);
  ASSERT_EQ("xwalk-extension://ioccpomhcpklobebcbeohnmffkmcokbm/"
...
TEST(ProcessExtensions, SingleExtensionWithBgPage) {
  ...
  manifest.reset(base::JSONReader::Read(manifest_txt));
  ...
  ASSERT_EQ(
      "xwalk-extension://jijhlkpcmmeckhlgdipjhnchhoabdjae/"
...
TEST(PrepareUserDataDir, CustomPrefs) {
  ...
  base::FilePath prefs_file =
      temp_dir.path().AppendASCII(xwalk::kInitialProfile).Append(
          xwalk::kPreferencesFilename);
  std::string prefs_str;
  ASSERT_TRUE(base::ReadFileToString(prefs_file, &prefs_str));
  scoped_ptr<base::Value> prefs_value;
  prefs_value.reset(base::JSONReader::Read(prefs_str));
  ...
  base::FilePath local_state_file =
      temp_dir.path().Append(xwalk::kLocalStateFilename);
