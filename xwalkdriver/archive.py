...
def DownloadXwalk(revision, dest_dir, site=Site.CONTINUOUS):
  """Downloads the packaged Xwalk from the archive to the given directory.

  Args:
    revision: the revision of Xwalk to download.
    dest_dir: the directory to download Xwalk to.
    site: the archive site to download from, default to the continuous one.

  Returns:
    The path to the unzipped Xwalk binary.
  """
  def GetZipName():
    if util.IsWindows():
      return 'xwalk-win32'
  ...
  def GetXwalkPathFromPackage():
    if util.IsWindows():
      ...
      return 'xwalk'
  zip_path = os.path.join(dest_dir, 'xwalk-%s.zip' % revision)
  ...
  return os.path.join(dest_dir, GetZipName(), GetXwalkPathFromPackage())
