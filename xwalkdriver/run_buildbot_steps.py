...
"""Runs all the buildbot steps for XwalkDriver except for update/compile."""
...
GS_CHROMEDRIVER_BUCKET = 'gs://xwalkdriver'
GS_CHROMEDRIVER_DATA_BUCKET = 'gs://xwalkdriver-data'
GS_CHROMEDRIVER_RELEASE_URL = 'http://xwalkdriver.storage.googleapis.com'
...
SERVER_LOGS_LINK = (
    'http://xwalkdriver-data.storage.googleapis.com/server_logs')
...
def _ArchivePrebuilts(commit_position):
  ...
  zip_path = util.Zip(os.path.join(xwalk_paths.GetBuildDir(['xwalkdriver']),
                                   'xwalkdriver'))
...
def _ArchiveServerLogs():
  """Uploads xwalkdriver server logs to google storage."""
  util.MarkBuildStepStart('archive xwalkdriver server logs')
...
def _DownloadPrebuilts():
  ...
  util.MarkBuildStepStart('Download latest xwalkdriver')
  ...
  util.Unzip(zip_path, xwalk_paths.GetBuildDir(['host_forwarder']))
...
def _GetVersion():
  """Get the current xwalkdriver version."""
  with open(os.path.join(_THIS_DIR, 'VERSION'), 'r') as f:
    return f.read().strip()


def _GetSupportedXwalkVersions():
  """Get the minimum and maximum supported Xwalk versions.

  Returns:
    A tuple of the form (min_version, max_version).
  """
  # Minimum supported Xwalk version is embedded as:
  # const int kMinimumSupportedXwalkVersion[] = {27, 0, 1453, 0};
  with open(os.path.join(_THIS_DIR, 'xwalk', 'version.cc'), 'r') as f:
    lines = f.readlines()
    xwalk_min_version_line = [
        x for x in lines if 'kMinimumSupportedXwalkVersion' in x]
  xwalk_min_version = xwalk_min_version_line[0].split('{')[1].split(',')[0]
  with open(os.path.join(xwalk_paths.GetSrc(), 'xwalk', 'VERSION'), 'r') as f:
    xwalk_max_version = f.readlines()[0].split('=')[1].strip()
  return (xwalk_min_version, xwalk_max_version)
...
def _ArchiveGoodBuild(platform, commit_position):
  """Archive xwalkdriver binary if the build is green."""
  assert platform != 'android'
  util.MarkBuildStepStart('archive build')

  server_name = 'xwalkdriver'
  if util.IsWindows():
    server_name += '.exe'
  zip_path = util.Zip(os.path.join(xwalk_paths.GetBuildDir([server_name]),
                                   server_name))

  build_name = 'xwalkdriver_%s_%s.%s.zip' % (
...
def _WasReleased(version, platform):
  ...
  result, _ = slave_utils.GSUtilListBucket(
      '%s/%s/xwalkdriver_%s.zip' % (GS_CHROMEDRIVER_BUCKET, version, platform),
      [])
  return result == 0
...
def _MaybeRelease(platform):
  ...
  candidate_pattern = re.compile(
      r'.*/xwalkdriver_%s_%s\.(\d+)\.zip$' % (platform, version))
  candidates = []
  ...
  # the xwalkdriver bucket instead of bumping up the release version number.
  ...
  for commit_position in candidates:
    ...
    if android_result == 'failed':
      ...
      candidate = 'xwalkdriver_%s_%s.%s.zip' % (
...
def _Release(build, version, platform):
  ...
  release_name = 'xwalkdriver_%s.zip' % platform
  ...
  if util.IsLinux():
    ...
    server_path = os.path.join(temp_dir, 'xwalkdriver')
...
def _MaybeUploadReleaseNotes(version):
  ...
  fixed_issues = []
  query = ('https://code.google.com/p/xwalkdriver/issues/csv?'
           'can=1&q=label%%3AXwalkDriver-%s&colspec=ID%%20Summary' % version)
  ...
  for issue in csv.reader(issues):
    ...
    desc = issue[1]
    labels = issue[2]
    labels.remove('XwalkDriver-%s' % version)
  ...
  new_notes = '----------XwalkDriver v%s (%s)----------\n%s\n%s\n\n%s' % (
      version, datetime.date.today().isoformat(),
      'Supports Xwalk v%s-%s' % _GetSupportedXwalkVersions(),
      '\n'.join(fixed_issues),
      old_notes)
...
def _MaybeUpdateLatestRelease(version):
  ...
  # Check if xwalkdriver was released on all supported platforms.
...
def _CleanTmpDir():
  ...
  for file_name in os.listdir(tmp_dir):
    ...
    if file_name.startswith('xwalkdriver_'):
