...
Status PerformanceLogger::HandleTraceEvents(
  ...
  if (method == "Tracing.tracingComplete") {
    ...
    if (buffer_usage >= 0.99999) {
      ...
      std::string err("Xwalk's trace buffer filled while collecting events, "
...
Status PerformanceLogger::CollectTraceEvents() {
  ...
  // https://code.google.com/p/xwalkdriver/issues/detail?id=997 for details.
  // TODO(samuong): find other commands where we don't need the response.
  bool wait_for_response = false;
  if (session_->xwalk) {
    const BrowserInfo* browser_info = session_->xwalk->GetBrowserInfo();
    if (browser_info->browser_name == "xwalk" && browser_info->build_no < 2245)
