...
TEST(ParseBrowserInfo, BlinkVersionContainsSvnRevision) {
  std::string data("{\"Browser\": \"Xwalk/37.0.2062.124\","
                   " \"WebKit-Version\": \"537.36 (@181352)\"}");
  BrowserInfo browser_info;
  Status status = ParseBrowserInfo(data, &browser_info);
  ASSERT_TRUE(status.IsOk());
  ASSERT_EQ("xwalk", browser_info.browser_name);
...
TEST(ParseBrowserString, KitKatWebView) {
  BrowserInfo browser_info;
  Status status =
      ParseBrowserString(false, "Version/4.0 Xwalk/30.0.0.0", &browser_info);
  ASSERT_TRUE(status.IsOk());
...
TEST(ParseBrowserString, LollipopWebView) {
  BrowserInfo browser_info;
  Status status = ParseBrowserString(true, "Xwalk/37.0.0.0", &browser_info);
  ASSERT_TRUE(status.IsOk());
  ASSERT_EQ("webview", browser_info.browser_name);
...
TEST(ParseBrowserString, AndroidXwalk) {
  BrowserInfo browser_info;
  Status status =
      ParseBrowserString(true, "Xwalk/39.0.2171.59", &browser_info);
  ASSERT_TRUE(status.IsOk());
  ASSERT_EQ("xwalk", browser_info.browser_name);
...
TEST(ParseBrowserString, DesktopXwalk) {
  BrowserInfo browser_info;
  Status status =
      ParseBrowserString(false, "Xwalk/39.0.2171.59", &browser_info);
  ASSERT_TRUE(status.IsOk());
  ASSERT_EQ("xwalk", browser_info.browser_name);
  ASSERT_EQ("39.0.2171.59", browser_info.browser_version);
  ASSERT_EQ(2171, browser_info.build_no);
