...
// TODO(craigdh): Remove once Xwalkdriver no longer supports pre-m33 Xwalk.
const char kXwalkCmdLineFileBeforeM33[] = "/data/local/xwalk-command-line";
const char kXwalkCmdLineFile[] = "/data/local/tmp/xwalk-command-line";
...
Status Device::SetUp(const std::string& package,
  ...
  } else if (package.compare("org.chromium.xwalk.shell") == 0) {
    // XwalkShell
    known_activity = ".XwalkShellActivity";
    device_socket = "xwalk_shell_devtools_remote";
    command_line_file = "/data/local/tmp/xwalk-shell-command-line";
    exec_name = "xwalk_shell";
  } else if (package.find("xwalk") != std::string::npos &&
             package.find("webview") == std::string::npos) {
    // Xwalk.
    known_activity = "com.google.android.apps.xwalk.Main";
    device_socket = "xwalk_devtools_remote";
    command_line_file = kXwalkCmdLineFileBeforeM33;
    exec_name = "xwalk";
  ...
  */
  if (!use_running_app) {
    ...
    if (!command_line_file.empty()) {
      // If Xwalk is set as the debug app it looks in /data/local/tmp/.
      ...
      if (command_line_file == kXwalkCmdLineFileBeforeM33) {
        status = adb_->SetCommandLineFile(
            serial_, kXwalkCmdLineFileBeforeM33, exec_name, args);
        Status status2 = adb_->SetCommandLineFile(
            serial_, kXwalkCmdLineFile, exec_name, args);
        if (status.IsError() && status2.IsError())
          return Status(kUnknownError,
              "Failed to set Xwalk's command line file on device " + serial_);
      } else {
        status = adb_->SetCommandLineFile(
