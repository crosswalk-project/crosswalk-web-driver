...
const std::string kVersionPrefix = "Xwalk/";
...
Status ParseBrowserString(bool has_android_package,
  ...
  if (browser_string.find(kVersionPrefix) == 0u) {
    ...
    if (version_parts.size() != 4 ||
      ...
      return Status(kUnknownError, "unrecognized Xwalk version: " + version);
    }

    if (build_no != 0) {
      browser_info->browser_name = "xwalk";
  ...
  return Status(kUnknownError,
                "unrecognized Xwalk version: " + browser_string);
}
...
Status ParseBlinkVersionString(const std::string& blink_version,
  ...
  // Xwalk OS reports its Blink revision as a git hash. In this case, ignore it
  // and don't set |blink_revision|. For Xwalk (and for Xwalk OS) we use the
