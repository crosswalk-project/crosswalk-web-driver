...
This script is called from xwalk/test/xwalkdriver/run_all_tests.py and reports
results using the buildbot annotation scheme.

For XwalkDriver documentation, refer to http://code.google.com/p/xwalkdriver.
"""
...
def _Run(java_tests_src_dir, test_filter,
         xwalkdriver_path, xwalk_path, log_path, android_package_key,
         verbose, debug):
  ...
  Args:
    ...
    xwalkdriver_path: path to XwalkDriver exe.
    xwalk_path: path to Xwalk exe.
    log_path: path to server log.
    android_package_key: name of Xwalk's Android package.
  ...
  sys_props = ['selenium.browser=xwalk',
               'webdriver.xwalk.driver=' + os.path.abspath(xwalkdriver_path)]
  if xwalk_path:
    sys_props += ['webdriver.xwalk.binary=' + os.path.abspath(xwalk_path)]
  if log_path:
    sys_props += ['webdriver.xwalk.logfile=' + log_path]
  if android_package_key:
    android_package = constants.PACKAGE_INFO[android_package_key].package
    sys_props += ['webdriver.xwalk.android_package=' + android_package]
    if android_package_key == 'xwalkdriver_webview_shell':
      android_activity = constants.PACKAGE_INFO[android_package_key].activity
      android_process = '%s:main' % android_package
      sys_props += ['webdriver.xwalk.android_activity=' + android_activity]
      sys_props += ['webdriver.xwalk.android_process=' + android_process]
  if test_filter:
  ...
  return _RunAntTest(
      test_dir, 'org.openqa.selenium.xwalk.XwalkDriverTests',
...
def main():
  ...
  parser.add_option(
      '', '--xwalkdriver', type='string', default=None,
      help='Path to a build of the xwalkdriver library(REQUIRED!)')
  parser.add_option(
      '', '--xwalk', type='string', default=None,
      help='Path to a build of the xwalk binary')
  parser.add_option(
      '', '--log-path',
  ...
  if options.xwalkdriver is None or not os.path.exists(options.xwalkdriver):
  ...
  if options.android_package:
    ...
    if options.xwalk_version != 'HEAD':
      parser.error('Android does not support the --xwalk-version argument.')
  ...
  try:
    ...
    java_tests_src_dir = os.path.join(xwalk_paths.GetSrc(), 'xwalk', 'test',
                                      'xwalkdriver', 'third_party',
                                      'java_tests')
    if (not os.path.exists(java_tests_src_dir) or
      ...
      print ('"%s" is empty or it doesn\'t exist. ' % java_tests_src_dir +
             'Need to map <xwalk-svn>/trunk/deps/third_party/webdriver to '
             'xwalk/test/xwalkdriver/third_party/java_tests in .gclient.\n'
             'Alternatively, do:\n'
             '  $ cd xwalk/test/xwalkdriver/third_party\n'
    ...
    for filter in test_filters:
      results += _Run(
          ...
          test_filter=filter,
          xwalkdriver_path=options.xwalkdriver,
          xwalk_path=util.GetAbsolutePathOfUserPath(options.xwalk),
