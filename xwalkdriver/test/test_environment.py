...
class BaseTestEnvironment(object):
  ...
  def __init__(self, xwalk_version='HEAD'):
    """Initializes a desktop test environment.

    Args:
      xwalk_version: Optionally a xwalk version to run the tests against.
    """
    self._xwalk_version = xwalk_version
  ...
  def GetDisabledJavaTestMatchers(self):
    ...
    return _EXPECTATIONS['GetDisabledTestMatchers'](
        self.GetOS(), self._xwalk_version)
...
class AndroidTestEnvironment(DesktopTestEnvironment):
  ...
  def __init__(self, package, xwalk_version='HEAD'):
    super(AndroidTestEnvironment, self).__init__(xwalk_version)
